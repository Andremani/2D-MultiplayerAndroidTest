...
namespace Andremani.TwoDMultiplayerAndroidTest
{
    ...
    */
    ...
    public class NetRoomManager : NetworkRoomManager
    {
        ...
        /// <param name="roomPlayerObject">The room player object for this connection.</param>
        ...
        public override GameObject OnRoomServerCreateGamePlayer(NetworkConnectionToClient conn, GameObject roomPlayerObject)
        {
            ...
            if(!gamePlayer.TryGetComponent<Player>(out Player playerComponent))
            {
                Debug.LogError("No Player component attached to gamePlayer Prefab!");
                return null;
            }
            if(!roomPlayerObject.TryGetComponent<NetRoomPlayer>(out NetRoomPlayer roomPlayer))
            {
                Debug.LogError("No NetRoomPlayer component attached to roomPlayer Prefab!");
                return null;
            }

            if(roomPlayer.Nickname == "")
            {
                playerComponent.Nickname = "Noname#" + UnityEngine.Random.Range(0, 10000);
            }
            else
            {
                playerComponent.Nickname = roomPlayer.Nickname; //TODO: Check for same nicknames
            }
            
            OnServerAddedGamePlayer?.Invoke(conn, playerComponent);
